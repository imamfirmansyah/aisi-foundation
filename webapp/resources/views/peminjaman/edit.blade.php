...
@section('content')
    ...
    <div class="add-listing-section">
        <form method="POST" action="{{ route('peminjaman.set.edit') }}" enctype="multipart/form-data">
        @csrf
        <!-- Headline -->
            ...
            <input type="hidden" name="id" value="{{ !empty($peminjaman->id) ? $peminjaman->id : 0 }}">

            <div class="row with-forms">
                <div class="col-md-6">
                    <h5>Tanggal Peminjaman</h5>
                    <input name="tgl_peminjaman" type="text" id="date-picker-start" value="{{ @$peminjaman->tgl_pinjam }}" placeholder="Tanggal Peminjaman">
                </div>
                <div class="col-md-6">
                    <h5>Tanggal Pengembalian</h5>
                    <input name="tgl_pengembalian" type="text" id="date-picker-end" value="{{ @$peminjaman->tgl_kembali }}" placeholder="Tanggal Pengembalian">
                </div>
            </div>
            ...
            <div class="row with-forms">
                ...
                <div class="col-md-12">
                    <h5>Keterangan</h5>
                    <textarea name="keterangan" cols="40" rows="3" placeholder="Keterangan Peminjaman">{{ @$peminjaman->keterangan }}</textarea>
                </div>
                @if(Auth::user()->role === "STAFF")
                    <div class="col-md-12">
                        ...
                        <select class="chosen-select-no-single" >
                            ...
                            <option {{ @$data['peminjaman']->status == 'DIPINJAM' ? 'selected' : '' }}>DIPINJAM</option>
                            <option {{ @$data['peminjaman']->status == 'DIKEMBALIKAN' ? 'selected' : '' }}>DIKEMBALIKAN</option>
                        </select>
                    </div>
                @endif
