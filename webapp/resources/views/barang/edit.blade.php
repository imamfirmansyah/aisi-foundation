...
@section('content')
    ...
    <div class="add-listing-section aisi-form">
        <form method="POST" action="{{ route('barang.update', ['id' => $data->id] ) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="row with-forms">
                ...
                <div class="col-md-12">
                    ...
                    <select name="jenis_barang" class="chosen-select-no-single">
                        ...
                        <option value="ATK" {{ ( $data->jenis_barang ) === "ATK" ? 'selected' : '' }}>ATK</option>
                        <option value="ELEKTRONIK" {{ ( $data->jenis_barang ) === "ELEKTRONIK" ? 'selected' : '' }}>ELEKTRONIK</option>
                        <option value="RUANGAN" {{ ( $data->jenis_barang ) === "RUANGAN" ? 'selected' : '' }}>RUANGAN</option>
                        <option value="LAIN-LAIN" {{ ( $data->jenis_barang ) === "LAIN-LAIN" ? 'selected' : '' }}>LAIN-LAIN</option>
                    </select>
                ...
                <div class="col-md-12">
                    <h5>Foto Barang</h5>
                    <img width="260" height="auto" src="{{ url( 'storage/barang/'. $data->foto ) }}">
